...
function bots(VK, token) {  
	...
	/*==========================================================================================================*/
	    if(!uid[1][message.user]){ 
			...
			if(acc[aid] != 1){
				return message.send(`
					...
					❗ В официальной группе: vk.com/catcherbot
		...
		if(acc[cid].users[user_id(message.user)].registr == 1){ 
			...
			if(aid != 1){return message.send(`✅ Пройти регистрацию нужно в основной группе: [catcherbot|CatcherBOT]`);} 
			if(message.$from.type != 'user'){
	...
	*/
	...
	vk.updates.hear(/^(?:cmd|команды|кмд|помощь|✅ Помощь|@catcherbot ✅ Помощь)/i, (message) => { 
		...
		if(aid != 1){
			return message.send(`
				...
				‼ Только в основной группе: vk.com/catcherbot
				- - - - - -  
	...
	vk.updates.hear(/^(?:альянс инфо)$/i, (message) => {  
		return message.send(`
		    ...
		    CatcherBOT будет функционировать в вашей группе, 
			...
			Чтобы создать Альянс, нужно написать в ЛС ГРУППЫ vk.com/catcherbot
			- - -
			"Создать [TOKEN] [ID]"
			[TOKEN] - Токен вашей группы.
			[ID] - цифровой ИД группы.

			По всем вопросам писать - vk.com/coder_chater
		`);
	});
	/*==========================================================================================================*/
	...
	/*==========================================================================================================*/
	vk.updates.hear(/^(?:✅ Герои|герои|@catcherbot ✅ Герои)/i, (message) => {  
	...
	/*==========================================================================================================*/
 	...
 	vk.updates.hear(/^(?:бот)/i, (message) => {
		...
		Империал Бот - это принципиально новый бот от всеми известного создателя, из под чьих ловких рук вышло не мало легенд этой отрасли! В этом проекте были учтены недостатки прошлых и мы представляем вам увлекательную РПГ игру с невероятным количеством общения, сражений, строительства, развития! 
 		...
 		&#128128; Developer(coder): [coder_chater|Razmik Baghdasaryan]

 		&#128122; Дизайнер: [coder_chater|Razmik]

 		👻 Креативщик: [coder_chater|Razmik]
	...
	/*==========================================================================================================*/  
 	...
 	vk.updates.hear(/^(?:ответ)\s?([0-9]+)?\s?([^]+)?/i, (message) => {
 		...
 		vk.api.call('messages.send', {
			peer_id: acc[cid].users[ids].id,
			message: `✅ » Администрация ответила вам:\n-> ${message.$match[2]}`
		});
 	...
 	vk.updates.hear(/^(?:unban)\s?([0-9]+)?/i, (message) => {
 		...
 		vk.api.call('messages.send', {
			peer_id: acc[cid].users[ids].id,
			message: `✅ » Администрация разблокировал Ваш аккаунт.`
		});
